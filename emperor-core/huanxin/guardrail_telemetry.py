...
Integrates with huanxin.tracer.Tracer to emit span events for all guardrail
...
Integration pattern — callbacks / decorators:
  GuardrailTelemetry.emit(event) is called from Huanxin.execute_task after
...
logger = logging.getLogger("huanxin.guardrail_telemetry")
...
class GuardrailTelemetry:
    ...
    (via huanxin.tracer).  If no span is active, events are still
