#!/usr/bin/env python3
"""幻炘AI CLI — 多领域 AI Agent 管理系统命令行工具。

Usage:
    huanxin serve                    启动 Dashboard 服务器
    huanxin task <prompt>            提交任务
    huanxin status                   查看系统状态
    huanxin ministers                列出所有大臣
    huanxin evolve                   手动触发进化
    huanxin alerts                   查看活跃告警
    huanxin self-evolve              运行自进化闭环（离线/真实 PR）
    huanxin --version                显示版本号
"""
...
# 优先级：命令行显式参数 > 环境变量 HUANXIN_HOST/HUANXIN_PORT > 下列默认值。
...
def _resolve_serve_host(cli_host: str | None) -> str:
    """解析监听地址：命令行 > HUANXIN_HOST > 默认 0.0.0.0。"""
    if cli_host:
        return cli_host
    env_host = os.environ.get("HUANXIN_HOST", "").strip()
    if env_host:
        return env_host
...
def _resolve_serve_port(cli_port: int | None) -> int:
    """解析监听端口：命令行 > HUANXIN_PORT > 默认 8000。

    环境变量非法（非数字/越界）时回退到默认端口并告警，不让容器直接崩。
    """
    if cli_port:
        return int(cli_port)
    env_port = os.environ.get("HUANXIN_PORT", "").strip()
    if env_port:
        try:
            port = int(env_port)
        except ValueError:
            print(f"  [警告] HUANXIN_PORT={env_port!r} 不是合法整数，回退到 {DEFAULT_SERVE_PORT}")
            return DEFAULT_SERVE_PORT
        if 1 <= port <= 65535:
            return port
        print(f"  [警告] HUANXIN_PORT={port} 越界(1-65535)，回退到 {DEFAULT_SERVE_PORT}")
    return DEFAULT_SERVE_PORT
...
def cmd_serve(args: argparse.Namespace) -> None:
    ...
    # HuanxinConfig 的 data_dir/court_path 会自行读取 HUANXIN_DATA_DIR /
    # HUANXIN_COURT_PATH，这里只负责把最终 host/port 灌进配置。
    cfg = HuanxinConfig()
    cfg.api_host = host
    cfg.api_port = port
...
def cmd_status(args: argparse.Namespace) -> None:
    ...
    print()
    print(f"  {_c('幻炘AI', _BOLD)} v{VERSION}")
    ...
    if sched is not None and hasattr(sched, "state"):
        from huanxin.court.scheduler import SchedulerState
        state = sched.state
...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog="huanxin",
        description="幻炘AI — 多领域 AI Agent 管理系统",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=textwrap.dedent("""\
            示例:
              huanxin serve                       启动 Dashboard
              huanxin serve --port 8080           指定端口
              huanxin task "计算 2+3"             提交任务
              huanxin task --domain math "计算 pi" 指定领域
              huanxin status                      查看状态
              huanxin ministers                   大臣列表
              huanxin evolve                      手动进化
              huanxin alerts                      告警列表
              huanxin self-evolve                运行自进化闭环（离线确定/真实 PR）
        """),
    )

    parser.add_argument(
        "--version", action="version", version=f"huanxin {VERSION}"
    )
    ...
    serve_parser.add_argument(
        "--host", default=None,
        help=f"监听地址 (未指定时读 HUANXIN_HOST，默认: {DEFAULT_SERVE_HOST})",
    )
    serve_parser.add_argument(
        "--port", type=int, default=0,
        help=f"监听端口 (未指定时读 HUANXIN_PORT，默认: {DEFAULT_SERVE_PORT})",
    )
    ...
    se_parser.add_argument("--memory-path", default=None,
                           help="经验记忆持久化路径（默认 huanxin/court/memory.json）")
