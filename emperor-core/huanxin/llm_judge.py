...
要使"系统是否变好"可证伪，请在 `huanxin.eval_bench` 中使用
:class:`~huanxin.eval_bench.judges.DeterministicJudge`（基于显式
`gold_validator` 的离线权威裁判）。本模块的 `evaluate()` 已支持通过
环境变量 `HUANXIN_JUDGE_MODE` 切换裁判后端：
...
* ``"llm"`` —— 委托给 `eval_bench` 的 `LLMBackedJudge`（真实 LLM 裁判）。
  若未配置 `HUANXIN_LLM_API_KEY`，会**显式告警并回退**到启发式（带 warning），
  绝不静默假装高分。
...
logger = logging.getLogger("huanxin.llm_judge")
...
_HEURISTIC_WARNING = (
    ...
    "不代表事实正确性；请使用 huanxin.eval_bench 的 DeterministicJudge / LLMBackedJudge 获得可证伪信号。"
)


def _resolve_judge_mode(explicit: Optional[str] = None) -> str:
    """解析生效的裁判模式：显式参数 > ``HUANXIN_JUDGE_MODE`` > 默认 deterministic。"""
    mode = (explicit or os.getenv(JUDGE_MODE_ENV, DEFAULT_JUDGE_MODE)).strip().lower()
    if mode not in ("deterministic", "llm", "heuristic"):
        logger.warning("[llm_judge] 未知 HUANXIN_JUDGE_MODE=%r，回退为 deterministic", mode)
...
def _evaluate_accuracy(output: str, expected: str, mode: str = "deterministic") -> DimensionScore:
    ...
    * ``mode in ("deterministic", "llm")`` —— 委托给 ``huanxin.eval_bench`` 的
...
def _evaluate_accuracy_llm(llm_judge_inst: Any, output: str, expected: str) -> DimensionScore:
    """用真实 LLM 裁判评估 accuracy（仅在 HUANXIN_JUDGE_MODE=llm 且可用时）。"""
    try:
        case = _BenchCase(
            input=expected or "",
            expected=expected or "",
...
class LLMJudge:
    ...
    def evaluate(
        ...
        Note (P0.6):
            默认（``HUANXIN_JUDGE_MODE=deterministic``）下，``accuracy`` 维度
            委托给 ``huanxin.eval_bench.DeterministicJudge``，不再用关键词重叠
        ...
        if mode == "llm":
            try:
                from huanxin.eval_bench.judges import LLMBackedJudge as _LLMBacked
