...
Each minister is a self-contained AI agent that:
    ...
    3. Reports structured results back to the Sovereign
...
logger = logging.getLogger("huanxin.court.minister")
...
@dataclass
class Edict:
    """An imperial edict (task dispatch) from the Sovereign to a Minister."""
    edict_id: str
    intent: str
    context: dict[str, Any] = field(default_factory=dict)
    priority: int = 5            # 1-10, higher = more urgent
    deadline_ms: Optional[float] = None  # Soft deadline in millis; None = no deadline
    minister: str = ""           # Set by Sovereign when dispatching


@dataclass
class Memorial:
    """A minister's memorial (response report) back to the Sovereign."""
    edict_id: str
    minister: str
    state: MinisterState
    success: bool
...
class ExperienceRecord:
    ...
    confidence: float
    feedback_score: float = 0.0  # Updated post-hoc by user/Sovereign
    learned_pattern: str = ""
...
class Minister:
    ...
    async def receive_edict(self, edict: Edict) -> Memorial:
        """Receive an edict from the Sovereign and return a memorial.

        This is the primary entry point used by the Sovereign.
    ...
    def record_feedback(self, edict_id: str, score: float) -> None:
        """Record external feedback (from Sovereign or user) for a past edict.
