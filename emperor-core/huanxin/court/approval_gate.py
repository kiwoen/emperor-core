...
早就有一个完整的 :class:`~huanxin.approval.ApprovalEngine` 审批引擎，自进化循环
却从未使用它。
...
logger = logging.getLogger("huanxin.court.approval_gate")
...
class WritebackApprovalGate:
    ...
    Args:
        approval_engine: 既有的 :class:`~huanxin.approval.ApprovalEngine` 实例；
