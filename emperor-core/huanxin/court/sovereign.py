"""
Sovereign (天子) — the sovereign orchestrator of the Imperial Court.

The Sovereign receives user intents, analyzes them, dispatches edicts to
...
from huanxin.court.reflection import ReflectionConsensus

logger = logging.getLogger("huanxin.court.sovereign")
...
class CourtPhase(Enum):
    ...
    SYNTHESIZING = auto()    # Sovereign weighing all memorials
...
@dataclass
class Decree:
    """Final imperial decree — the Sovereign's synthesized response to the user.

    This is the structured output returned after all ministers have
    deliberated and the Sovereign has weighed their counsel.
    """
    decree_id: str
    intent: str
    success: bool
...
class ImperialCourt:
    """The Sovereign's cabinet — manages all ministers and coordinates
    ...
    def install_minister(self, minister: Minister) -> None:
        ...
        logger.info("[Sovereign] Appointed %s (%s) to the court",
    ...
    def dismiss_minister(self, name: str) -> bool:
        ...
        if name in self.ministers:
            ...
            logger.info("[Sovereign] Dismissed %s from the court", name)
            return True
        return False
    ...
    async def receive_petition(
        ...
        """The Sovereign receives a petition (user request) and returns a decree.
        ...
        Full pipeline:
            ...
            4. Synthesize — Sovereign weighs all memorials and decides
        ...
        def _emit(phase: CourtPhase, **detail: Any) -> None:
            if on_progress:
                try:
                    ...
                    logger.debug(
                        "[Sovereign] on_progress 回调失败（已转为可观测，非致命）",
                        exc_info=True,
                    )

        logger.info(
            "[Sovereign] Receiving petition: decree=%s intent='%s'",
            decree_id, intent[:80],
        )
        ...
        logger.info("[Sovereign] Court session with: %s", selected)
        ...
        for m in memorials:
            if isinstance(m, Memorial):
                ...
                logger.warning("[Sovereign] A minister failed: %s", m)
        ...
        logger.info(
            "[Sovereign] Decree %s issued: success=%s confidence=%.2f ministers=%d",
    ...
    async def _synthesize(
        ...
        - If ministers disagree: present both sides + Sovereign decides
    ...
    async def _ingest_into_kg(self, decree: Decree) -> None:
        ...
        try:
            ...
            for m in decree.memorials:
                ingest_text = (
                    f"Sovereign decreed: {decree.intent} "
            ...
            logger.debug("[Sovereign] KG ingestion skipped (non-critical)")
    ...
    async def _run_evolution_if_needed(self) -> None:
        ...
        try:
            ...
            for event in report.actions_taken:
                logger.info(
                    "[Sovereign] Evolution: %s → %s (merit=%.2f→%.2f)",
                    event.minister,
                    event.action,
            ...
            logger.debug("[Sovereign] Evolution cycle skipped (non-critical)")
    ...
    def send_feedback(
        ...
        """The Sovereign (or user) provides feedback on a minister's work."""
...
# ------------------------------------------------------------------
# Standalone Sovereign wrapper (for backward compatibility)
# ------------------------------------------------------------------


class Sovereign:
    """Convenience wrapper: Sovereign = ImperialCourt + auto-install + evolution.
