...
与 :class:`~huanxin.self_evolve.GenomeDrivenExecutor` 的关键区别：

  * GenomeDrivenExecutor 用「基因质量」直接**伪造**成败信号（不真正解题）；
  * RealTaskExecutor 先用 :class:`~huanxin.court.offline_solver.OfflineSolver`
...
from huanxin.court.fitness import FitnessSignal
from huanxin.court.genome_quality import true_quality
from huanxin.court.offline_solver import OfflineSolver

logger = logging.getLogger("huanxin.court.real_executor")
...
def build_default_llm() -> Optional[Callable[..., str]]:
    ...
    后端解析统一交给 :func:`huanxin.core.llm._resolve_backends_from_env`，与
    emperor 主路径（:func:`huanxin.core.llm.build_manager_from_env`）共用同一份
    ...
    try:
        from huanxin.core.llm import _resolve_backends_from_env, _safe_int, _safe_float
    except Exception:
