...
真实 :class:`~huanxin.vcs.git_channel.GitWriteChannel`（配好 gh 凭据）即可，
编排与安全闸门不变。

典型用法::

    from huanxin.court.court import Court, CourtConfig
    from huanxin.court.circuit_breaker import CircuitBreaker, PromotionGate
    from huanxin.self_evolve import SelfEvolutionEngine, GenomeDrivenExecutor, default_ministers
...
from huanxin.court.fitness import FitnessSignal, RealTaskFitness
...
from huanxin.court.memory import CourtMemory, memory_from_memorial
from huanxin.court.rollback import RollbackManager
from huanxin.court.resource_guard import ResourceBudget, ResourceBudgetExceeded
from huanxin.court.safety_gate import SafetyContext, SafetyGate, default_safety_gate
...
logger = logging.getLogger("huanxin.self_evolve")

# 基因「真实质量」统一来源（见 huanxin/court/genome_quality，安全闸与引擎共用）。
from huanxin.court.genome_quality import (  # true_quality re-exported via __all__
    OPT_CONFIDENCE,
    OPT_TEMPERATURE,
    true_quality,
)
...
def real_default_tasks() -> List[SimulatedTask]:
    ...
    :class:`~huanxin.court.offline_solver.OfflineSolver` 真正算出答案，从而
