...
when the Huanxin is started, providing out‑of‑the‑box self‑healing.

Usage:
    from huanxin.healing import HealingAction
    from huanxin.healing_actions import restart_scheduler, emergency_evolve
...
def _get_emperor() -> Optional[Any]:
    """Retrieve the current Huanxin instance (if any)."""
    try:
        from huanxin.core import Huanxin
        # This is a global singleton pattern; adjust if your app uses multiple.
        # For now, we assume the Huanxin is accessible via a global or module‑level variable.
