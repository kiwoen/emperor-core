...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("huanxin.log", encoding="utf-8"),
    ],
)
logger = logging.getLogger("huanxin")


def print_banner(config: HUANXINConfig) -> None:
    """Display the HUANXIN startup banner."""
    banner = f"""
╔══════════════════════════════════════════════════════════════╗
║                                                              ║
║     ██╗ █████╗ ██████╗ ██╗   ██╗██╗███████╗                ║
...
async def run_cli(integration: SystemIntegration) -> None:
    """Run HUANXIN in interactive CLI mode via the integrated pipeline."""
    ...
    while True:
        ...
        if result["success"]:
            ...
            print(f"HUANXIN [{domain}] > {output}")
        else:
            error = result.get("error", "Unknown error")
            print(f"HUANXIN > [ERROR] {error}")


async def main() -> None:
    """Initialize and start HUANXIN using SystemIntegration."""
    # Load configuration
    config = load_config()
    print_banner(config)

    # Initialize LLM engine
    from huanxin.core.llm import init_llm
    init_llm(config)
    ...
    if config.web_server_enabled:
        ...
        from huanxin.api.server import start_server
