...
Key features:
    ...
    - Fallback strategies (RETRY / REJECT / DELEGATE_TO_HUANXIN)
    - Timeout protection (handoff deadline enforcement)
    - Handoff chain tracking (full audit trail of minister → minister flow)

Usage:
    from huanxin.handoff import HandoffProtocol, HandoffRequest, HandoffContext
...
logger = logging.getLogger("huanxin.handoff")
...
class FallbackStrategy(Enum):
    ...
    DELEGATE_TO_HUANXIN = "delegate_to_emperor"  # Escalate to Huanxin
...
class HandoffProtocol:
    ...
    def _apply_fallback(
        ...
        if strategy == FallbackStrategy.DELEGATE_TO_HUANXIN:
            return HandoffResult(
                handoff_id=request.handoff_id,
