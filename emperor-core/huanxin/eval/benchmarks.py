...
Benchmarks:
    HuanxinBench      — 20 题，覆盖全部 12 种内置能力
...
# ══════════════════════════════════════════════════════════════════
# 1. HuanxinBench — 20 题覆盖 12 种内置能力
# ══════════════════════════════════════════════════════════════════

class HuanxinBench(Benchmark):
    ...
    def build_cases(self) -> List[BenchmarkCase]:
        return [
            ...
            # text (2)
            BenchmarkCase("JB_txt_01", "统计 'Hello World from Huanxin!' 的字符数", capability="text",
                          expected_route="text"),
            ...
            BenchmarkCase("JB_hash_01", "计算 'password123' 的 MD5 哈希值", capability="hash",
                          expected_route="hash"),
            BenchmarkCase("JB_hash_02", "对 'Huanxin Core v1.0' 做 SHA-256 摘要", capability="hash",
                          expected_route="hash"),

            # json_tool (2)
            BenchmarkCase("JB_json_01", '验证这段 JSON 是否合法：{"name":"Huanxin","version":"1.0"}',
...
def get_benchmark(name: str) -> Optional[Benchmark]:
    ...
    Args:
        name: 基准名称 — "huanxin" / "router" / "multistep" / "selfhealing"
