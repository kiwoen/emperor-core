...
* :class:`LLMBackedJudge` — 可选的、opt-in 的 LLM 裁判，经由
  :mod:`jarvis.core.llm` 调用真实模型。**若未配置 API key，它会直接
  raise 而非静默回退成假高分。**

工厂 :func:`get_judge` 依据环境变量 ``EMPEROR_JUDGE_MODE``（默认
``"deterministic"``）选择实现。
"""
...
from jarvis.eval_bench.criteria import EvalCase, EvalResult

logger = logging.getLogger("jarvis.eval_bench.judges")
...
LLM_API_KEY_ENV = "EMPEROR_LLM_API_KEY"
...
def resolve_judge_mode(explicit: Optional[str] = None) -> str:
    ...
    优先级：显式参数 > ``EMPEROR_JUDGE_MODE`` > 默认 ``"deterministic"``。
    未知值回退为 deterministic 并记录警告。
    """
    mode = (explicit or os.getenv(JUDGE_MODE_ENV, DEFAULT_JUDGE_MODE)).strip().lower()
    if mode not in VALID_MODES:
        logger.warning("[eval_bench] 未知 EMPEROR_JUDGE_MODE=%r，回退为 deterministic", mode)
...
class LLMBackedJudge:
    ...
    本裁判是**严格**的：若 ``EMPEROR_LLM_API_KEY`` 未设置（或 LLM 客户端无法
    ...
    def __init__(
        ...
        if not self._api_key:
            ...
            raise JudgeUnavailableError(
                f"EMPEROR_LLM_API_KEY 未配置，LLMBackedJudge 无法运行真实裁判；"
                f"请勿静默回退为关键词重叠的假高分。"
            )
        ...
        from jarvis.core.llm import LLMEngine, LLMConfig
...
def get_judge(mode: Optional[str] = None) -> Any:
    ...
    * ``"heuristic"``     → :class:`DeterministicJudge`（离线确定性；
                            旧的"关键词重叠"路径只存在于 :mod:`jarvis.llm_judge`
                            并已被显式标注为非权威）
