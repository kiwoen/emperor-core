...
| **接口与集成** | mcp-python-sdk、OpenAI Agents SDK(tracing)、LangGraph(可视化) | `court_api`(172 路由，**7 个 Flask 风格 `<param>` 404** 附录 A §3.5-9)、`api/`(双份)、`hermes/`(双总线)、`cli`(双套,`emperor_cli` 未引用)、`domains/`(8 骨架) | 一般 | 集成层重复+契约缺陷。**最后一公里**：修 7 路由 + 收敛双份 |
...
| # | 位置 | 机制 | 性质 | 判定 |
|---|---|---|---|---|
| S1 | `emperor.py:260-269` | `SmartRouter`(P2.9) 导入被 `try/except ImportError` 吞掉 → `_smart_router` 恒 None，`:838-841` 永不执行 | 能力路由层**已发布(CHANGELOG 有 P2.9)却静默失效** | 🔴 **真·静默失效**（明确） |
...
| 编号 | 失效特性 | 证据 | 类型 |
|---|---|---|---|
| S1 | P2.9 SmartRouter 静默失效 | `emperor.py:261/268/838-841` | ImportError 吞掉 |
| F1 | `court_api.py` 7 个 Flask 风格 `<param>` 路由在 FastAPI 下 404 | `:1626/1634/1664/1673/1916/1933/1976` | 契约 bug |
| F2 | `PromptGuard` 检出 dangerous 只 `logger.warning`，不拦截 | `emperor.py:824-829` | 拦截失效 |
| F3 | `HallucinationGuard` 检出不拦截不纠正，原文照返 | `emperor.py:1030-1041` | 拦截失效 |
...
graph TD
    ...
    subgraph REFACTOR["🟠 改造（接通/替换信号源）"]
        LLM["emperor.execute_task<br/>接真实 LLM + 贯穿 task_id"]
...
| `CostTruth` | 可观测成本 | F | `cost_tracker`, `emperor` | 真实 `task_id` 关联 | `cost_per_success` 非空率 **100%** |
...
| **四插件** `plugin.py`+`plugin_system.py`+`plugin_marketplace.py`+`plugins/` | `plugin_system.py`（manifest+热加载，最完整）+ `plugin.py`（生命周期钩子，主链路用） | 统一 `PluginManager` 命名（解决 `emperor.py:206-215` 的 `as PluginSystemManager` 别名冲突）；`plugin_marketplace` 合并进 `plugin_system` 的 registry | 单 `PluginManager`，marketplace 作其内部注册表 |
...
| P3.4 集成层收敛 | P3 | P0.1, P1.9 | 双 API/双总线/双 CLI 合并 | 单入口；`emperor_cli` 接入 entry point |
...
graph LR
    subgraph EXEC["执行"]
        A["execute_task<br/>emperor.py:663"]
...
| B10 | **幻觉检测 → 拦截** | `emperor.py:1023-1042` | `check()` 走 `detect_sync`（启发式）；检出后仅 `logger.warning`，`result["response"]` 原样返回。`SelfCorrectionLoop.correct()` 全库无调用点 |
| B11 | **注入检测 → 拦截** | `emperor.py:824-829` | `if _pg_result.level == "dangerous": logger.warning(...)` 之后**没有 return/raise**，执行继续 |
...
| 5 | PromptGuard 检出 dangerous 后不拦截，仅打日志 | 🔴 高 | `emperor.py:824-829` |
| 6 | HallucinationGuard 检出后不拦截不纠正 | 🟠 中高 | `emperor.py:1030-1041` |
| 7 | `ToolRegistry.execute_tool` 零校验直接执行 | 🔴 高 | `tools/registry.py:148-193` |
| 8 | 单文件巨兽：`dashboard_html.py` 6997 行、`court_api.py` 3901 行、`evolution.py` 2079 行、`emperor.py` 1745 行 | 🟠 中 | `wc -l` |
