"""Tests for Huanxin ↔ PluginManager integration.

Verifies that lifecycle events are dispatched at the right points
during Huanxin's workflow.
"""

import pytest

from huanxin.core import Huanxin, HuanxinConfig
from huanxin.plugin import LifecycleEvent, Plugin
...
class TestHuanxinPluginIntegration:
    def test_plugins_property_has_metrics_plugin(self):
        emp = Huanxin()
        # Huanxin now eagerly registers a MetricsPlugin on init
    ...
    def test_init_dispatched_to_plugin(self):
        emp = Huanxin()
        tracker = _Tracker()
        # Register plugin BEFORE creating Huanxin would be ideal, but
        # the Huanxin has a fresh PluginManager on construction.
        # So the test is: Huanxin's __init__ dispatches ON_INIT.
