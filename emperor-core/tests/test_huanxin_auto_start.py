"""Tests for Huanxin auto-start (one-command live dashboard)."""
...
class TestAutoSeedMinisters:
    ...
    def test_ensure_default_ministers_respects_max_ministers(self):
        cfg = HuanxinConfig(max_ministers=3)
...
class TestServeAutoStart:
    ...
    def test_serve_with_auto_seed(self):
        cfg = HuanxinConfig(auto_seed_ministers=True, auto_schedule=False)
        emp = Huanxin(config=cfg)
        assert len(emp.court.active_ministers) == 0
        emp.serve(port=9999)
        assert len(emp.court.active_ministers) > 0
    ...
    def test_serve_with_auto_schedule(self):
        cfg = HuanxinConfig(auto_seed_ministers=True, auto_schedule=True)
        emp = Huanxin(config=cfg)
        emp.serve(port=9999)
        assert len(emp.court.active_ministers) > 0
        assert emp.scheduler.state.name == "RUNNING"
    ...
    def test_serve_without_auto_features(self):
        cfg = HuanxinConfig(auto_seed_ministers=False, auto_schedule=False)
        emp = Huanxin(config=cfg)
        emp.serve(port=9999)
        assert len(emp.court.active_ministers) == 0
        assert emp.scheduler.state.name == "IDLE"
...
def test_one_command_live_dashboard():
    """End-to-end: Huanxin().serve() → seeded ministers + running scheduler."""
    cfg = HuanxinConfig(
        auto_seed_ministers=True,
        auto_schedule=True,
    ...
    from huanxin import court_api as ca
