"""
Per-Agent Eval Suite — isolated evaluation of each HUANXIN agent.
...
Integrates with existing:
    - huanxin.eval.EvalSuite / EvalRunner
    - huanxin.llm_judge.LLMJudge (accuracy sub-dimension)
    - huanxin.hallucination_guard.HallucinationGuard
    - huanxin.tools.validator.ToolCallValidator / ToolCallLog
    - huanxin.cost_tracker.CostTracker

Usage:
    from huanxin.evaluation.agent_eval import AgentEvalSuite, eval_all_agents
...
logger = logging.getLogger("huanxin.evaluation.agent_eval")
...
class AgentEvalSuite:
    """Isolated evaluation suite for a single HUANXIN agent.

    Each agent is evaluated on:
    1. Synthetic inputs generated per its responsibility domain
    2. LLM-as-Judge accuracy scoring via huanxin.llm_judge.LLMJudge
    3. Hallucination detection via huanxin.hallucination_guard.HallucinationGuard
    4. Tool call success tracking via huanxin.tools.validator
