"""
HUANXIN API Server — FastAPI + WebSocket.
...
logger = logging.getLogger("huanxin.api")
...
async def lifespan(_app: FastAPI) -> AsyncIterator[None]:
    ...
    logger.info("HUANXIN API server started")
    yield
    ...
    logger.info("HUANXIN API server stopped")
...
# 可选 Token 鉴权：未设 HUANXIN_API_TOKEN 则不生效（向后兼容）
add_token_auth(app)
...
async def status():
    ...
    return StatusResponse(
        name=config_ref.name if config_ref else "HUANXIN",
...
async def search_memory(query: str = "", limit: int = 20):
    ...
    from huanxin.core.orchestrator import TaskResult
...
async def websocket_endpoint(ws: WebSocket):
    ...
    greeting = {
        "type": "system",
        "message": f"Connected to HUANXIN v{config_ref.version if config_ref else '0.1.0'}.",
        "timestamp": datetime.now(timezone.utc).isoformat(),
...
async def start_server(orchestrator: Any, config: Any) -> None:
    """Start the API server (called from HUANXIN main entry point)."""
...
async def start_server_with_events(integration: Any, config: Any) -> None:
    ...
    Args:
        ...
        config: The HUANXIN config object.
    """
    ...
    from huanxin.events.stream import EventStreamManager
