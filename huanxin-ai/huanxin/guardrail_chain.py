...
Before this file existed, ``huanxin/tool_guard.py``, ``huanxin/loop_guard.py``,
``huanxin/bounded_autonomy.py`` and ``huanxin/hallucination_guard.py`` were fully
implemented, fully unit-tested — and almost entirely **unreachable** from
``Huanxin.execute_task``.  The guards existed on paper only.  This chain closes
that gap.
...
2. **Shadow first.**  ``HUANXIN_GUARDRAIL_MODE`` defaults to ``"shadow"``:
...
logger = logging.getLogger("huanxin.guardrail_chain")

#: Environment variable controlling the enforcement posture.
GUARDRAIL_MODE_ENV = "HUANXIN_GUARDRAIL_MODE"
...
class GuardrailChain:
    ...
    Args:
        tool_guard: ``huanxin.tool_guard.ThreeTierGuardEnhancement``.
        loop_guard: ``huanxin.loop_guard.AgentLoopGuard``.
        bounded_autonomy: ``huanxin.bounded_autonomy.BoundedAutonomyEngine``.
        hallucination_guard: ``huanxin.hallucination_guard.HallucinationGuard``.
        telemetry: ``huanxin.guardrail_telemetry.GuardrailTelemetry`` sink.
        mode: Explicit mode override; defaults to the ``HUANXIN_GUARDRAIL_MODE``
    ...
    def mode(self) -> GuardrailMode:
        ...
        ``HUANXIN_GUARDRAIL_MODE`` without restarting the process — unless an
    ...
    def _check_loop_guard(self, task_id: str) -> GuardrailCheck:
        ...
        and is already called once by ``Huanxin.execute_task``.  Calling it
    ...
    def _run_guard(self, name: str, fn: Any, *args: Any) -> GuardrailCheck:
        ...
        if guard_obj is None:
            ...
            return GuardrailCheck(
                ...
                detail=f"{name} is not wired into the Huanxin",
            )
