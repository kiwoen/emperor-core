...
Allows HUANXIN modules to call external MCP tools via Hermes topics.
Acts as a bridge from Hermes → MCP (stdio or HTTP transport).

Architecture:
    ┌──────────────┐   Hermes Bus    ┌──────────────┐   MCP/stdio   ┌───────────┐
    │  HUANXIN       │◄──────────────►│ HermesMCP     │◄────────────►│ External   │
...
logger = logging.getLogger("huanxin.hermes_agent.client")
...
class MCPProcess:
    ...
    async def initialize(self) -> dict:
        return await self._rpc("initialize", {
            ...
            "capabilities": {},
            "clientInfo": {"name": "huanxin-hermes", "version": "1.0.0"},
        })
...
class HermesMCPClient:
    ...
    async def _on_request(self, msg) -> None:
        from huanxin.hermes.bus import MessageType
