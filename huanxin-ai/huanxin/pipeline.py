...
def _create_daily_brief_pipeline():
    ...
    from huanxin.capability import _news_handler, _handle_text
...
def _create_health_check_pipeline():
    ...
    from huanxin.capability import _weather_handler

    return (ServicePipeline("健康检查", auto_retry=False)
        .add_stage(Stage(
            name="系统健康",
            handler=lambda ctx: (
                __import__('huanxin.health', fromlist=['get_system_health'])
                .get_system_health()
            ),
