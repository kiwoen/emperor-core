...
logger = logging.getLogger("huanxin.knowledge")
...
def _extract_entities(text: str) -> list[tuple[str, str]]:
    ...
    # Dot-separated: huanxin.codex.engine → "domain" / "module"
    for match in re.finditer(r"\b([a-z_]+(?:\.[a-z_]+)+)\b", text):
        name = match.group(1)
        if name not in seen:
            entity_type = "domain" if name.startswith("huanxin.") else "module"
...
class KnowledgeGraph:
    ...
    Usage:
        kg = KnowledgeGraph()
        await kg.ingest("HUANXIN helps with Code Review using Vector Engine")
