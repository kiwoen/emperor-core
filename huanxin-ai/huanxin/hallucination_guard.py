...
与 huanxin.prompt_guard.PromptGuard 共同构成 Pre+Post LLM guardrail 体系：
  - PromptGuard  : 前置——拦截 Prompt Injection（指令覆盖/角色劫持/提示提取/越狱/编码混淆）
  - HallucinationGuard : 后置——拦截 LLM 输出幻觉（无依据声明/事实性错误）

Usage:
    from huanxin.hallucination_guard import HallucinationDetector, SelfCorrectionLoop
...
logger = logging.getLogger("huanxin.hallucination_guard")
