"""
HUANXIN Orchestrator — the master controller.

This is the brain of HUANXIN. It receives natural language intents,
...
logger = logging.getLogger("huanxin.orchestrator")


class Domain(Enum):
    """All domains HUANXIN operates in."""
...
class ExecutionMode(Enum):
    ...
    COURT:  route through the Imperial Court (Huanxin → ministers → decree)
    """
...
class DomainRegistry:
    ...
    at runtime without restarting HUANXIN.
    """
...
class Orchestrator:
    """The master controller of HUANXIN.
    ...
    def workflow(self) -> Any:
        ...
        if self._workflow is None:
            from huanxin.core.workflow import WorkflowEngine
    ...
    async def execute_stream(self, user_input: str) -> AsyncIterator[dict[str, Any]]:
        ...
        # Court mode: delegate to Huanxin pipeline
        if (
            self.execution_mode == ExecutionMode.COURT
            and self.imperial_court is not None
        ):
    ...
    def set_court_mode(self, enabled: bool = True) -> None:
        ...
        When enabled, all user intents are routed through the Huanxin
    ...
    async def _execute_with_court(self, user_input: str, intent: Intent) -> TaskResult:
        """Route through the Imperial Court: petition → Huanxin → ministers → decree.
    ...
    def load_all_domains(self) -> None:
        ...
        domain_module_names = [
            "huanxin.domains.personal",
            "huanxin.domains.research",
            "huanxin.domains.engineering",
            "huanxin.domains.creator",
            "huanxin.domains.security",
            "huanxin.domains.health",
            "huanxin.domains.finance",
            "huanxin.domains.home",
        ]
