"""Configuration system for 幻炘AI.

Loads settings from huanxin.yaml (JSON-inside-YAML, compatible with Python stdlib).
Falls back to sensible defaults when no config file exists.
First run auto-generates huanxin.yaml with all defaults.
