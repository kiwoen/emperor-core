...
Usage:
    python -m huanxin.court_api                       # default: 127.0.0.1:8000
    python -m huanxin.court_api --port 9000            # custom port
    python -m huanxin.court_api --config court.yaml    # config-driven
...
logger = logging.getLogger("huanxin.court_api")
...
from huanxin.governance_agent import GovernanceAgent, GovernanceRule, RulePriority
from huanxin.bounded_autonomy import ActionZone, ActionSpace, BoundedAutonomyEngine
...
from huanxin.prompt_guard import PromptGuard, ScanResult

# RBAC module imports
from huanxin.rbac import RBACEngine, Permission, Role, intent_to_permission

# 可选启用的 Token 鉴权中间件（HUANXIN_API_TOKEN 未设则不生效）
...
class ChatRequest(BaseModel):
    ...
    system: str = Field(
        default="你是 幻炘AI —— 一个会自我进化的 AI 助手，回答简洁、准确、有帮助。",
        description="系统提示词",
    )
...
# ══════════════════════════════════════════════════════════════════
# Module-level scheduler state (shared with Sovereign.serve)
# ══════════════════════════════════════════════════════════════════
...
"""Module-level reference to the SovereignConfig (huanxin.yaml AppConfig).
Injected by Sovereign.serve() via configure_app()."""


def configure_app(emperor_config=None):
    """Inject huanxin.yaml AppConfig so API endpoints can read/write it.

    Args:
        emperor_config: An AppConfig instance from huanxin.yaml.
    """
...
def _get_llm_manager():
    ...
    if _llm_manager is None:
        from huanxin.core.llm import build_manager_from_env
...
def create_app(
    ...
    """
    app = FastAPI(title="Sovereign Court API", version="0.1.0")

    # 多用户存储层初始化（幂等建表；数据落在 $HUANXIN_DATA_DIR/emperor.db，数据卷持久化）
    auth_store.init_db()
    # 强制登录 + 单用户部署：启动时种入唯一管理员账号。
    # 凭据来源：HUANXIN_ADMIN_USER / HUANXIN_ADMIN_PASS；若未设则回退到
    # HUANXIN_API_TOKEN 的值作为初始密码（保证升级后不会锁死），再不行则随机生成并打印告警。
    admin_user = (os.getenv("HUANXIN_ADMIN_USER", "admin") or "admin").strip()
    admin_pass = (os.getenv("HUANXIN_ADMIN_PASS", "") or "").strip() or (os.getenv("HUANXIN_API_TOKEN", "") or "").strip()
    if not admin_pass:
        admin_pass = secrets.token_urlsafe(16)
        logger.warning(
            "HUANXIN_ADMIN_PASS 与 HUANXIN_API_TOKEN 均未设置，已生成随机管理员密码"
            "（仅本次启动有效，请尽快在 .env 固定 HUANXIN_ADMIN_PASS）：%s", admin_pass
        )
    auth_store.ensure_admin(admin_user, admin_pass)
    logger.info("已确保管理员账号存在：username=%s", admin_user)
    # 强制会话登录鉴权（详见 huanxin/api/token_guard.py）
    add_token_auth(
        app,
    ...
    # Inject RBAC Engine (may be overridden by Sovereign.serve())
...
document.getElementById('p').addEventListener('keydown',k=>{if(k.key==='Enter')go()});
    ...
    def dashboard_summary():
        ...
        from huanxin.health import get_uptime_seconds
    ...
    def evolution_learning_curve():
        ...
        """
        from huanxin.learning_curve import get_learning_curve
    ...
    def _run_evolution_worker(cycles: int):
        ...
        if emperor is None:
            _evo_job["last_error"] = "Sovereign not available"
    ...
    def api_get_config():
        ...
        # Prefer _emperor_config (injected by Sovereign.serve), fallback to app.extra
    ...
    def api_set_theme(req: ThemeRequest):
        """Set dashboard theme and persist to huanxin.yaml."""
        import json as _json
        import os as _os
        ...
        # Persist to huanxin.yaml
        config_path = "huanxin.yaml"
    ...
    def health_check():
        ...
        from huanxin.health import get_system_health
    ...
    def dashboard_live():
        ...
        from huanxin.capability import _weather_handler, _news_handler
    ...
    def evals_run():
        ...
        try:
            from huanxin.eval import create_builtin_suites
    ...
    def _get_template_manager():
        ...
        if mgr is None:
            from huanxin.capability import get_template_manager
    ...
    def execute_pipeline(body: PipelineExecuteRequest):
        """执行服务流水线"""
        try:
            from huanxin.pipeline import pipeline_registry, PipelineStatus
    ...
    def add_pipeline_schedule():
        ...
        try:
            from huanxin.pipeline import pipeline_scheduler
    ...
    def pipeline_monitor_summary():
        ...
        from huanxin.pipeline_monitor import pipeline_monitor
    ...
    def dashboard_search(q: str = "", limit: int = 5, request: Request = None):
        ...
        if emperor is None:
            raise HTTPException(status_code=503, detail="Sovereign not available")
    ...
    def healing_trigger(action_name: str, request: Request):
        ...
        from huanxin.healing import HealingRecord
    ...
    def api_governance_rules():
        ...
        from huanxin.governance_store import governance_store
    ...
    def api_alert_rules():
        ...
        from huanxin.alert_rule_store import alert_rule_store
    ...
    def mcp_register_server(request: Request, payload: MCPServerRegister):
        ...
        if emp is None:
            raise HTTPException(status_code=503, detail="Sovereign not available")
        mgr = emp.mcp_manager
        from huanxin.mcp_client import MCPServerConfig
    ...
    from huanxin.state_machine import list_workflow_templates, execute_workflow
...
# ══════════════════════════════════════════════════════════════════
# CLI entry: python -m huanxin.court_api
# ══════════════════════════════════════════════════════════════════
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Sovereign Court API server")
    ...
    print(f"Sovereign Court API -> http://{args.host}:{args.port}")
