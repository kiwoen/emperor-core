...
logger = logging.getLogger("huanxin.llm.config")
...
class LLMConfig(BaseModel):
    ...
    # The canonical multi-backend implementation lives in huanxin.core.llm
    ...
    def from_env(cls, **overrides: Any) -> "LLMConfig":
        ...
        Mirrors ``huanxin.core.llm.LLMConfig.from_env`` so the emperor entry
    ...
    def to_core(self) -> Any:
        """Convert to a ``huanxin.core.llm.LLMConfig`` dataclass for delegation."""
        from huanxin.core.llm import LLMConfig as CoreLLMConfig
    ...
    def from_core(cls, core_cfg: Any) -> "LLMConfig":
        """Build a pydantic ``LLMConfig`` from a ``huanxin.core.llm.LLMConfig``."""
        return cls(
