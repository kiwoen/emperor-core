...
| 护栏谎报 | `huanxin/core.py:810-831`：`:815` 上报 `action="blocked"`，但 `:824` 仅 `logger.warning`，**不中止** | 比无护栏更危险：telemetry 显示拦截，执行继续 |
...
| 路由静默失效 | `huanxin/core.py:261` `from huanxin.model_router import SmartRouter`；`:268` `except ImportError: self._smart_router = None`；`:838-841` `self._smart_router.classify(...)` 永不执行 | **`huanxin/model_router.py` 文件不存在** → `ImportError` 永远触发 → `_smart_router` 恒 `None`；CHANGELOG 却写了 P2.9「已发布」 |
...
- **测试**：`tests/test_core.py` 新增 `test_prompt_guard_blocks_dangerous`——注入已知危险 prompt，断言返回 `success=False` 且 `error` 含 `prompt_injection_blocked`，且**未调用 LLM 后端**（用 mock 计数）。
...
- **文件**：`huanxin/core.py`（主执行 `run_task` 路径）、`huanxin/guardrail_telemetry.py`
- **改动**：
...
- **文件**：
  ...
  - `huanxin/core.py:259-269`（修静默 import：失败要**显式报警**）
...
- **改动**：
  ...
  2. `core.py:268` 把 `except ImportError` 改为 `logger.error("SmartRouter 缺失，路由降级为功勋第一")`（不再静默 `=None`）。
...
- **测试**：`tests/test_model_router.py`（新知识，覆盖 classify/get_tier/get_fallback）+ `tests/test_core.py` 断言 `core.py:838-841` 在 SmartRouter 存在时**实际执行**（非 `_smart_router=None`）。
...
pytest -q                                    # 全量回归
pytest tests/test_core.py::test_prompt_guard_blocks_dangerous -q   # 单点验证
...
2. **若你确认开始执行**：我可立即按 P0.1 起手——用 Cursor/Trae 生成 `core.py` 阻断分支 + `tests/test_core.py` 单测，经 VSCode review 后，**由你在 GitHub 上开 PR**（人类闸门）。我不会自行直推。
