...
- 新增 `huanxin.governance_agent` 模块
...
- 新增 `huanxin.bounded_autonomy` 模块
...
#### P1.1: 能力命中统计环形饼图
- 新增 `huanxin.capability_stats` 模块，通过装饰器 `@track_capability_hit` 拦截 `dispatch()` 调用
...
- 新增 `huanxin.context_compressor` 模块
- 长对话历史智能压缩管理
- 支持保留最近 N 条消息

#### P2.7: ContextVersioning — 上下文版本控制
- 新增 `huanxin.context_versioning` 模块
- 不可变状态快照 + 回滚
- 支持注册可版本化组件（Plugins / Templates）
- 启动时自动创建基线快照

#### P2.8: MultiModelRouter — 多模型路由
- 新增 `huanxin.multi_model` 模块
...
- 新增 `huanxin.model_router.SmartRouter`
- 能力感知路由，支持回退链
- 支持 YAML 配置文件 `model_routing.yaml`

#### P2.10: CostPerSuccessTracker
- 新增 `huanxin.cost_per_success` 模块
- 追踪每次成功运行的成本（CPS 指标）
...
- 新增 `huanxin.plugin_system.PluginManager`：热加载第三方插件
- 新增 `huanxin.plugin_marketplace.PluginMarketplace`：插件市场（安装/搜索/列表）
- 新增 `huanxin.plugins.MetricsPlugin`：生命周期事件采集

#### P3.10: TaskRouter — 任务路由器
- 新增 `huanxin.router.RouterEngine`
- 基于意图分类的多级路由
- `IntentClassifier` 支持 LLM 辅助分类

#### P3.11: WorkflowEngine — DAG 工作流
- 新增 `huanxin.workflow.WorkflowEngine`
- DAG 多步骤任务编排
- 支持步骤依赖和条件执行
...
- Dashboard HTML 单文件拆分为模块化 `huanxin/dashboard_html.py`（6300+ 行）
- Court API 独立为 `huanxin/court_api.py`（3500+ 行）
- `Huanxin.__init__` 延迟导入优化，加速启动
- 所有模块通过 `huanxin/__init__.py` 统一导出
- `pyproject.toml` 版本号从占位符更新为 `0.1.0`
- CLI 入口修复：`huanxin/huanxin = huanxin.cli:main`
