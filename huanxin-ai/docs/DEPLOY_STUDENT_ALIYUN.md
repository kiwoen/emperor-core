# huanxin-ai 学生实验部署 Runbook —— 阿里云版

> 面向：**学生个人实验**、**公网 IP 直连（不买域名、暂不备案）**、**有阿里云学生认证**。
> 目标：用阿里云「云工开物」学生计划 + 毕业设计包，把 huanxin-ai 一键 `docker compose up` 跑起来。
...
| ❌ 选 1 核 2G | ✅ 选 **2 核 4G**（huanxin-ai slim 镜像 + 运行时 + SQLite 容易吃满 1G） |
...
| 安全组 | 放通 **TCP 22（SSH）+ TCP 8000（huanxin-ai）**；80/443 暂不开 |
...
```bash
# 拉代码（约 10MB）
git clone https://github.com/kiwoen/huanxin-ai.git .
...
# 应返回：{"status":"ok","service":"huanxin-ai"}
...
```bash
cd /srv/huanxin-ai && git fetch origin master && \
...
```bash
ssh root@<公网IP> 'cd /srv/huanxin-ai && git fetch origin master && \
  git show origin/master:scripts/remote_deploy.sh | bash -s'
```
...
```bash
# 本机执行
ssh-keygen -t ed25519 -C "github-actions-deploy" -f ~/.ssh/huanxin_deploy -N ""
ssh-copy-id -i ~/.ssh/huanxin_deploy.pub root@<公网IP>   # 公钥装到服务器
cat ~/.ssh/huanxin_deploy                                 # 私钥全文 → 存为 DEPLOY_SSH_KEY
```
...
8000 端口直接暴露公网，建议至少加一层 Token 鉴权（我们已实现，由 `HUANXIN_API_TOKEN` 控制）。
...
> 不设置 `HUANXIN_API_TOKEN` = 完全不鉴权（向后兼容，仅限内网/实验）。设置后 `/health` 探针**始终放行**，不影响健康检查。
...
全部持久化数据（huanxin.db / audit.db / approval.db / outcome_records.json / 版本快照）都在命名卷 `huanxin-data` → 容器内 `/app/data`。

**备份**（服务器上执行，tar 打包卷到当前目录）：

```bash
docker run --rm -v huanxin-data:/data -v $(pwd):/backup busybox \
  tar czf /backup/huanxin-data-$(date +%Y%m%d).tar.gz -C /data .
ls -lh huanxin-data-*.tar.gz
```
...
```bash
docker run --rm -v huanxin-data:/data -v $(pwd):/backup busybox \
  tar xzf /backup/huanxin-data-YYYYMMDD.tar.gz -C /data
...
```cron
0 4 * * * cd /srv/huanxin-ai && docker run --rm -v huanxin-data:/data -v $(pwd):/backup busybox tar czf /backup/huanxin-data-$(date +\%Y\%m\%d).tar.gz -C /data . && find /srv/huanxin-ai/huanxin-data-*.tar.gz -mtime +14 -delete
```

> `docker compose down` **不会**删卷；只有 `docker compose down -v` 或手动 `docker volume rm huanxin-data` 才清数据。
...
- **B. 降配继续**：停实例 → 改成 **2 核 2G 经济型**（≈¥89/月）→ 重启，`huanxin-data` 卷自动挂载回来。
- **C. 换新配置重建**：`docker volume create huanxin-data2` 后迁移数据，旧实例释放。
...
| `curl /health` 一直 401 | 你设了 `HUANXIN_API_TOKEN` 但忘了带令牌；`/health` 本身不需要令牌，401 说明打错端口或路径 |
...
- [ ] （建议）`.env` 设 `HUANXIN_API_TOKEN`，鉴权生效
- [ ] 备份脚本进 crontab
