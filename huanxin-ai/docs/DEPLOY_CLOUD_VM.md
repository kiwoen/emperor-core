...
| 数据落盘      | 容器内 `/app/data`，由命名卷 `huanxin-data` 承载                                                                 |
...
| 类型 | 主机记录      | 记录值                     |
| -- | --------- | ----------------------- |
| A  | `sovereign` | `123.45.67.89`（你的公网 IP） |

等 1–5 分钟生效，验证：`ping huanxin.example.com` 能解析到你的 IP。
...
```bash
$ curl -f https://huanxin.example.com/health
# {"status":"ok","service":"huanxin-ai"}
```
...
`docker-compose.yml` 已把命名卷 `huanxin-data` 挂到容器 `/app/data`，  
容器**删了重建数据也不丢**。里面是：
...
```bash
$ docker volume inspect huanxin-data
...
```bash
# 备份（当前目录生成 huanxin-data.tar.gz）
$ cd /srv/huanxin-ai
$ docker run --rm -v huanxin-data:/data -v $(pwd):/backup busybox \
    tar czf /backup/huanxin-data.tar.gz -C /data .

# 恢复（会覆盖卷内同名文件；建议先停服务）
$ docker compose stop
$ docker run --rm -v huanxin-data:/data -v $(pwd):/backup busybox \
    tar xzf /backup/huanxin-data.tar.gz -C /data
...
```bash
$ mkdir -p /srv/backups
$ cat > /usr/local/bin/huanxin-backup.sh <<'EOF'
#!/bin/sh
...
OUT=/srv/backups
mkdir -p "$OUT"
docker run --rm -v huanxin-data:/data -v "$OUT":/backup busybox \
    tar czf "/backup/huanxin-data-$STAMP.tar.gz" -C /data .
# 清理 14 天前的备份
find "$OUT" -name 'huanxin-data-*.tar.gz' -mtime +14 -delete
EOF
$ chmod +x /usr/local/bin/huanxin-backup.sh

# 写入 crontab
$ ( crontab -l 2>/dev/null; echo "0 3 * * * /usr/local/bin/huanxin-backup.sh >> /var/log/huanxin-backup.log 2>&1" ) | crontab -
$ crontab -l    # 确认已写入
```
...
$ cat > /usr/local/bin/huanxin-watch.sh <<'EOF'
#!/bin/sh
if ! curl -fsS --max-time 10 http://localhost:8000/health > /dev/null; then
    echo "$(date '+%F %T') health FAILED, restarting" >> /var/log/huanxin-watch.log
    cd /srv/huanxin-ai && docker compose restart
fi
EOF
$ chmod +x /usr/local/bin/huanxin-watch.sh
$ ( crontab -l 2>/dev/null; echo "*/5 * * * * /usr/local/bin/huanxin-watch.sh" ) | crontab -
```

外部拨测（免费）：[UptimeRobot](https://uptimerobot.com) / 腾讯云云拨测  
→ 监控 `https://huanxin.example.com/health`，异常邮件/微信告警。
...
| 访问入口 | URL                                         |
| ---- | ------------------------------------------- |
| 健康检查 | `https://huanxin.example.com/health`        |
| 仪表盘  | `https://huanxin.example.com/dashboard`     |
| 接口文档 | `https://huanxin.example.com/docs`          |
| 法庭总览 | `https://huanxin.example.com/court/summary` |
