...
- ``huanxin/vcs/git_channel.py`` 作为例外不被扫描；
...
def test_detects_git_push_master():
    root = _write_tmp({"huanxin/evil.py": 'subprocess.run(["git","push","origin","master"])\n'})
    v = cwp.scan_paths(os.path.join(root, "huanxin"))
    assert v
...
def test_detects_gh_api_put_master():
    root = _write_tmp({
        "huanxin/evil.py": 'os.system("gh api repos/x/branches/master/protection -X PUT")\n',
    })
    v = cwp.scan_paths(os.path.join(root, "huanxin"))
    assert v
...
def test_ignores_legal_pr_creation():
    root = _write_tmp({
        "huanxin/ok.py": 'subprocess.run(["gh","api","repos/x/pulls","-f","base=master"])\n',
    })
    v = cwp.scan_paths(os.path.join(root, "huanxin"))
...
def test_ignores_git_channel_exception():
    ...
    root = _write_tmp({"huanxin/vcs/git_channel.py": content})
    v = cwp.scan_paths(os.path.join(root, "huanxin"))
    assert not v, f"git_channel.py 应被豁免: {v}"


def test_main_returns_1_on_violation():
    root = _write_tmp({"huanxin/evil.py": 'git push origin main\n'})
    code = cwp.main(["--root", root])
    assert code == 1


def test_main_returns_0_when_clean():
    root = _write_tmp({"huanxin/ok.py": 'x = 1  # 无害\n'})
    code = cwp.main(["--root", root])
    assert code == 0
