...
class TestHuanxinCreation:
    ...
    def test_with_config(self):
        cfg = HuanxinConfig(min_ministers=5, max_ministers=30)
        emp = Huanxin(config=cfg)
    ...
    def test_dashboard_empty(self):
        emp = Huanxin()
        d = emp.dashboard()
        assert "Huanxin Evolution Dashboard" in d
...
# "block".  Keep them in sync with huanxin/prompt_guard.py rule IDs.
...
def _emperor_with_minister(name: str = "alpha", domain: str = "math") -> Huanxin:
    """Build a minimal Huanxin with exactly one registered minister."""
    emp = Huanxin(config=HuanxinConfig())
...
class TestPromptGuardBlocks:
    ...
    def test_guard_mode_env_override(self, monkeypatch):
        """HUANXIN_PROMPT_GUARD_MODE=warn downgrades dangerous → suspicious."""
        monkeypatch.setenv("HUANXIN_PROMPT_GUARD_MODE", "warn")
        emp = _emperor_with_minister()
        result = emp.execute_task(DANGEROUS_PROMPTS[0], domain="math")
    ...
    def test_default_mode_is_block(self, monkeypatch):
        monkeypatch.delenv("HUANXIN_PROMPT_GUARD_MODE", raising=False)
        emp = _emperor_with_minister()
        result = emp.execute_task(DANGEROUS_PROMPTS[0], domain="math")
        assert result["status"] == "blocked"
...
class TestGuardrailChainWiring:
    ...
    def test_shadow_mode_never_blocks(self, monkeypatch):
        monkeypatch.setenv("HUANXIN_GUARDRAIL_MODE", "shadow")
        emp = _emperor_with_minister()
        result = emp.execute_task("What is 2+2?", domain="math")
        assert result["success"] is True
...
class TestSmartRouterWiring:
    ...
    def test_router_classifies_a_math_prompt(self):
        from huanxin.model_router import Capability
