# Emperor Core · 集成点地图（Integration Map）

> 目的：把 emperor-core 各子系统之间的集成边界画清楚——每个集成点写清
> **上游触发方 / 下游被调方 / 当前连通状态 / 缺口**。作为"接入闭环"的代码导航。
>
> 关联文档：`docs/SKILL_AUTHORING.md`（技能编写规范）、`jarvis/core/task_router.py`、
> `jarvis/codex/reviewer.py`。

---

## 0. 两条并行路由路径（必须分清）

emperor-core 的"主路由"不是单一的，而是**两套并存**：

| 路径 | 入口 | 调度器 | 末端执行者 | 当前状态 |
|------|------|--------|-----------|----------|
| **DIRECT** | `Orchestrator` | `IntentParser → DomainRegistry` | `DomainModule.handle()` | ✅ 已启用，代码审查接在此 |
| **COURT** | `Emperor` | `task_router.route_to_minister` | `Minister.receive_edict()` | ✅ 已启用，但**未触达 CodeReviewer** |

- `jarvis/core/orchestrator.py:55` `ExecutionMode`：`DIRECT` / `COURT`。
- `jarvis/core/orchestrator.py:94` `DomainModule.handle()` 是领域处理契约。
- `jarvis/core/task_router.py:92` `route_to_minister()` 是大臣派发契约。
- `jarvis/court/minister.py:388` `Minister.receive_edict()` 是大臣执行入口。
...
- **上游**：`jarvis/domains/engineering/__init__.py:29-60`（DIRECT 路径，意图命中 `审查|code review|review|audit` 时调用）
- **引擎**：`jarvis/codex/reviewer.py`（`CodeReviewer.review()` → `ReviewReport`；`to_markdown()` 兼容 `script-multi-review` 报告风格）
...
- **上游**：`Emperor`（`jarvis/emperor.py`）或 `Orchestrator` COURT 模式
- **调度**：`jarvis/core/task_router.py`
...
- **末端**：`jarvis/court/ministers/__init__.py:29` `create_ministers()` 八大臣
- **状态**：✅ 路由算法完整；⚠️ `TASK_TYPES["code"]` 含 `代码审查|审查代码|code review`，但命中后路由到的大臣（如工部尚书）`_handle` 用的是 LLM，不是 `CodeReviewer`
- **缺口**：见任务 #75

### IP-3 · 大臣体系（Minister）
- **基类**：`jarvis/court/minister.py:98` `Minister`
...
- **工厂**：`jarvis/court/ministers/__init__.py`（八大臣：丞相/御史大夫/太史令/工部尚书/太常/大司农/太卜/卫尉）
- **状态**：✅ 框架完整；❌ `WorksMinister`（工部尚书，代码域）未集成 `CodeReviewer`
- **缺口**：见任务 #71、#73

### IP-4 · 自进化引擎（Self-Evolution）
- **位置**：`jarvis/evolution/`（SelfEvolutionEngine）、`jarvis/emperor.py:evolve()`
- **学习信号**：真实任务成败（RealTaskExecutor + OfflineSolver）→ 基因微调 → `genome_state.json` 持久化
- **经验记忆**：`jarvis/court/memory.py`（CourtMemory，跨重启累积）
- **状态**：✅ 闭环通畅；⚠️ 见 `docs/...roadmap` 中 P0/P1 缺口（离线模式回报潜在、记忆无衰减/裁剪）
- **缺口**：非本轮重点（属长期演进）

### IP-5 · 鉴权 / 会话 / 用量（auth_store）
- **位置**：`jarvis/api/auth_store.py`（SQLite，数据卷持久化）
- **表**：users / conversations / messages / sessions / token_ledger
- **路由**：`jarvis/court_api.py:1056-1086`（/api/me、/api/conversations 全套 CRUD）
...
- **位置**：`jarvis/core/llm.py`（`LLMManager`）、`jarvis/llm/manager.py`
...
Emperor.evolve() ─→ SelfEvolutionEngine ─→ genome + CourtMemory ✅
...
1. **IP-1 → COURT 打通**：让"代码审查"意图经 `task_router` 命中大臣时，该大臣 `_handle` 调用
   ...
   `CodeReviewMinister`；并补 `jarvis/codex/SKILL.md`。
...
| 子系统 | 文件 | 行号锚点 |
|--------|------|----------|
| 代码审查引擎 | `jarvis/codex/reviewer.py` | `CodeReviewer.review` / `to_markdown` |
| 审查接入（DIRECT） | `jarvis/domains/engineering/__init__.py` | 29–60 |
| 主路由 | `jarvis/core/task_router.py` | `classify_task_type` / `route_to_minister` / `plan_dispatch` |
| 大臣基类 | `jarvis/court/minister.py` | `can_handle` / `receive_edict` / `_handle` |
| 八大臣工厂 | `jarvis/court/ministers/__init__.py` | `create_ministers` |
| 编排器 | `jarvis/core/orchestrator.py` | `ExecutionMode` / `DomainModule` |
| 鉴权/会话 | `jarvis/api/auth_store.py` | `list_conversations` / `add_message` |
| 技能规范 | `docs/SKILL_AUTHORING.md` | 八段式 + 最小模板 |
| 对话接口 | `jarvis/court_api.py` | `/api/chat` / `/api/conversations` |
