...
P2.1 的 :class:`~jarvis.vcs.git_channel.GitWriteChannel` 已经落实了「沙箱 +
人类审批门（只开 PR，绝不直推 master）」，但它此前**无条件**开 PR——
缺了「基准评测」这一环：一个让基准回归的突变也会被照样提交。

本模块补上这一环：写回前先跑 :func:`jarvis.eval_bench.run.run_suite`
得到 :class:`~jarvis.eval_bench.criteria.EvalReport`，由
...
典型用法::
    ...
    from jarvis.eval_bench.suites.canonical import canonical_suite
    from jarvis.vcs import GitWriteChannel, WritebackGate, WritebackBlocked
    ...
    try:
        ch.propose_change(
            repo="kiwoen/emperor-core", patch_text=diff, title="auto-absorb: ...",
...
logger = logging.getLogger("jarvis.vcs.writeback_gate")
