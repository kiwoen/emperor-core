...
Integration:
    - jarvis.governance_agent.GovernanceAgent → RED actions go through governance first
    - jarvis.approval.ApprovalEngine → YELLOW actions auto-trigger approval flow
...
logger = logging.getLogger("jarvis.bounded_autonomy")
...
class BoundedAutonomyEngine:
    ...
    Built-in Default Spaces (reference: jarvis.approval risk keywords):
    ...
    def __init__(
        ...
        Args:
            governance_agent: jarvis.governance_agent.GovernanceAgent for RED validation
            approval_engine: jarvis.approval.ApprovalEngine for YELLOW approval
    ...
    def evaluate(
        ...
        if zone == ActionZone.GREEN:
            ...
            if self._approval_engine is not None:
                try:
                    from jarvis.approval import classify_risk
